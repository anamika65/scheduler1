...
public class ParallelSchedulerTest {
	...
	public void testParallelScheduleWithSmallestDuration() {
		...
		Scheduler<OpBlock> parallelSche = new ParallelScheduler(activities,
				resMan, 72, resToIndex, ParallelSchedulerRule.INSTANCE);
		...
		assertTrue(schForResidents.get("1").size() == 3);
		assertTrue(schForResidents.get("2").size() == 0);
